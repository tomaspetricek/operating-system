...
void interrupts_init(void)
{
    ...
    move_exception_vector(); // copies exception vector table to address 0
    ENABLE_INTERRUPTS();
}
...
void irq_handler(void)
{
    ...
    for (j = 0; j < NUM_IRQS; j++)
    {
        ...
        {
            clearers[j]();
            ENABLE_INTERRUPTS(); // figure out why it is needed
            handlers[j];
            DISABLE_INTERRUPTS();
            return;
        }
...
void register_irq_handler(irq_number_t irq_num, interrupt_handler_f handler, interrupt_clearer_f clearer)
{
    ...
    {
        irq_pos = irq_num - 64; // what does 64 stand for?
